...
| 竞品 | 定位 | 本项目差异 |
|------|------|-----------|
| 牛客网 | 面试题库 + 模拟面试 | 通用题目，无个性化成长方案 |
| Offer先生 | 简历优化 + 面试辅导 | 人工服务，成本高 |
| LeetCode | 算法刷题 | 仅覆盖算法单项 |
...
| **简历业务** | 简历上传/解析/确认、人物画像生成 | 简历文件、人物画像 | 面试用户 |
...
| 解析简历 | 从简历中提取关键信息 | 系统（自动） |
| 生成人物画像 | 基于简历生成候选人画像 | 系统（自动） |
| 展示解析结果 | 展示解析后的简历信息和人物画像 | 系统 → 面试用户 |
| 确认简历 | 用户确认解析结果正确 | 面试用户 |
| 修改简历 | 用户修改解析不准确的信息 | 面试用户 |
| 重新解析 | 修改后重新解析简历 | 系统（自动） |
| 锁定简历 | 确认后锁定简历，进入面试 | 系统（自动） |
| 查看历史简历 | 用户查看自己上传过的简历列表 | 面试用户 |
| 删除简历 | 用户删除不需要的简历 | 面试用户 |

**功能关系**：
```
上传简历 ──▶ 解析简历 ──▶ 生成人物画像 ──▶ 展示解析结果
                                                      │
                                              ┌───────┴───────┐
                                              ▼               ▼
                                          确认简历        修改简历
...
| 薄弱技能 | 简历中较少体现或较弱的技能 |
| 经验水平 | 初级/中级/高级 |
| **岗位大类** | 自动识别的岗位类型（技术族TECH/产品族PRODUCT/设计族DESIGN等），支持多类别推断并排序 |
...
| 场景 | 超时时间 | 重试次数 | 降级策略 |
|------|---------|---------|---------|
| 简历解析 | 60s | 2 次 | 返回待确认的空画像，提示用户手动填写 |
...
**方案：LLM 解析（Spring AI Alibaba + 用户配置模型）**

| 环节 | 技术选型 | 说明 |
|------|---------|------|
| **文件解析** | Apache Tika | 将 PDF/TXT 转换为纯文本 |
| **结构化提取** | 用户配置的 L2 标准模型 | 智能提取简历关键信息 |
| **结果确认** | 用户确认机制 | 用户可修正解析结果 |
...
│              │                                                  │
│              ▼                                                  │
│  3. LLM 解析（用户配置的 L2 模型）                              │
│      └── 提取：基本信息、技能清单、项目经历、工作经历            │
│              │                                                  │
│              ▼                                                  │
│  4. 生成人物画像                                                │
│      └── 输出结构化 JSON                                         │
