...
### 6.2.2 面试出题安全入口（2026-08 第一阶段试点）

本阶段只迁移 `InterviewerAgent` 的模型出题链路。目标不是宣称能够彻底识别 Prompt 注入，
而是把外部内容固定为无指令权限的数据，并保证模型结果不能改变服务端面试流程。

```text
InterviewerAgent
→ LlmTaskInput（服务端参数 + DATA_ONLY 数据块）
→ LlmInterviewService.execute（INTERVIEWER 权限 + 既有 Redis 限流）
→ SafeLlmGateway（任务白名单、风险检测、Prompt 隔离、严格响应、有限重试）
→ LlmService（现有模型传输）
```

| 安全边界 | 当前实现 |
|---------|---------|
| **任务白名单** | 只有已注册的 `INTERVIEW_QUESTION_GENERATION` 可以通过本试点入口；任务类型、请求类型和响应类型必须同时匹配 |
| **可信指令** | system message 只由服务端任务定义、固定安全规则和 classpath Skill 组成，业务调用方不能传入自由 system prompt |
| **不可信数据** | 岗位画像、简历项目、辅助分析、历史问题和候选人回答统一序列化到 user message 的 `DATA_ONLY_JSON`，无论来源标签为何都没有指令权限 |
| **输入限制** | 单数据块最多 8000 个 Unicode 字符，单次最多 16 块、合计最多 12000 个 Unicode 字符，数据块 ID 在单次请求内不得重复 |
| **风险信号** | `PromptRiskDetectorChain` 组合可插拔检测器；首个本地实现检查明显规则覆盖、角色冒充、敏感数据外发、高风险动作、异常控制字符、零宽/双向控制字符、兼容字符混淆和一次性 Base64/十六进制指令 |
| **当前处置** | 检测器只报告信号；面试出题试点对任意输入或输出风险信号采取拒绝模型结果并使用固定模板的策略，检测器异常同样按失败关闭处理 |
| **严格响应** | 模型必须返回且只能返回 `{"question":"..."}`；拒绝代码围栏、缺失/额外/重复字段、非字符串、尾随 JSON 和超长内容，问题长度为 1～500 个 Unicode 字符 |
| **有限重试** | 只有普通响应结构错误使用完全相同的 system/user message 重试一次；第一次模型原文不会进入重试请求，安全或内容违规不重试 |
| **服务端状态** | `phase`、`topic`、`depth`、计数、主题切换和结束状态不属于模型响应，由现有 Skill 和服务端状态逻辑维护 |
| **失败兜底** | 输入缺失、限流、模型异常、检测拒绝或响应无效时，只返回按题型固定的只读模板，不拼接外部内容或模型原文 |

已接入的六类业务路径为：自我介绍、专业题/专业追问、主题切换、简历项目题、行为题和结束语。
各路径只发送当前生成问题需要的数据；结束语不发送岗位、简历、回答或分析内容。
